...
class SignaturePad {
    constructor(canvas, options = {}) {
        ...
        this.touchsStrategy = (options.touchsStrategy=='force'&&'ontouchstart' in window)?options.touchsStrategy:'speed'
    ...
    _strokeUpdate(event, isEnd = false) {
        ...
        if (!lastPoint || !(lastPoint && isLastPointTooClose) || (isEnd)) {
            const curve = this._addPoint(point, force, isEnd);
            if (!lastPoint) {
    ...
    _addPoint(point, force, isEnd = false) {
        ...
        if (_lastPoints.length > 2) {
            ...
            const widths = this._calculateCurveWidths(_lastPoints[1], _lastPoints[2], force, isEnd);
    ...
    _calculateCurveWidths(startPoint, endPoint, force = 0, isEnd = false) {
        let param = 1
        // console.log('param',param,force)

        if (this.touchsStrategy == 'force') {
            force = force === 0 ? this.lightLimit : force
            // force = Math.sqrt(force)
            // force=force-1.5
            force=force>this.lightLimit?force-this.lightLimit:force
            // force=force<=0.01?0.01:force
            // force=force<0.5?(force+0.4):force
            // force=force>1?force*force:force
            param = force / ((this.velocityFilterWeight*endPoint.velocityFrom(startPoint)  +
                (1 - this.velocityFilterWeight) * this._lastVelocity) + 1);
            // console.log('param-inner',force,endPoint.velocityFrom(startPoint))
